...
/**
 * A friendly implementation of a listener that receives events from RxBus.
 */
class FriendlyListener1 {
    ...
    private fun subscribeToPyngSubject(): Disposable {
        return RxBus.listen(MockPyngSubject::class.java).subscribe({
            ...
            System.err.println("received event in listener1 on thread: " + Thread.currentThread().id + " at " + timeStamp);
            when (it.subjectId) {
